...
autoketik = false;
prefixStatus = true;
targetpc = "6285888258313"; 
owner = "6285888258313"; 
fakeyoi = "Lolita Botシ︎"; 
HunterApi = "Ikyy69", 
xchillds = 'XChillDs' 
hardi = 'hardianto', 
valkey = "rivalgans", 
zeksapi = "tritoapriansyah",
dapapi = "anakasu",
nomorowner1 = "@6285888258313"; 
...
const checkRegisteredUser = (sender) => {
    ...
    const ownern = [`6285888258313@s.whatsapp.net`]
   ...
   const isRegister = register.includes(sender)
    ...
    const isOwner = ownern.includes(sender)
...
/*const reply = (teks) => {
			itsmevall.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/6285888258313?text=Assalamualaikum`}}})
		}*/
...
const fakewa = (teks) => {        
itsmevall.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/6285888258313?text=Assalamualaikum`}}})
		}
...
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\nV a L L. (@_itsmevall) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/6285888258313`,
                "mediaType": "10",
                "mediaUrl": `wa.me/6285888258313`,
                "thumbnailUrl": "https://j.top4top.io/p_20682m4yj0.jpg",
                "thumbnail": iye,
                "sourceUrl": `wa.me/6285888258313`,
...
async function sendFileFromUrl(from, url, caption, msg, men) {
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let _votes = JSON.parse(fs.readFileSync(`./lib/vote/${from}.json`));
        let fil = vote.map((v) => v.participant);
        let id_vote = sender ? sender : "6285888258313@s.whatsapp.net";
        if (fil.includes(id_vote)) {
          return mentions(
            "@" + sender.split("@")[0] + " Anda sudah vote",
            fil,
...
*「 𝘊𝘳𝘦𝘢𝘵𝘦𝘥 𝘉𝘺 trito シ︎ 」*`
         haitod = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `${prefix}menu`,buttonText:{displayText: `</️MENU`},type:1}]
              ...
              buttonsMessage = {footerText:'𝘊𝘳𝘦𝘢𝘵𝘦𝘥 𝘉𝘺 tritoシ︎', imageMessage: imageMsg,
...
groups = itsmevall.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "6285888258313@s.whatsapp.net"
        dtod = '6285888258313'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
...
case 'bugreport':
              ...
              itsmevall.sendMessage('6285888258313@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
...
case 'iggw':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
  reply('https://instagram.com/_itsmevall')
break
...
㋛${prefix}𝙩𝙬𝙞𝙩𝙩𝙚𝙧 <𝙪𝙧𝙡>
`
sendButMessage(from, menu, `𝘊𝘳𝘦𝘢𝘵𝘦𝘥 𝘉𝘺 trito シ︎`, [
          {
            buttonId: `${prefix}command`,
            buttonText: {
...
case 'katalog':
    ...
    "listMessage": {
      ...
      "footerText": " 𝘊𝘳𝘦𝘢𝘵𝘦𝘥 𝘉𝘺 trito シ︎"
    }
  }, {quoted: freply})
...
buttons = [{buttonId: `${prefix}listnulis`,buttonText:{displayText: `List Nulis🔖`},type:1}]
              ...
              buttonsMessage = {footerText:'𝘊𝘳𝘦𝘢𝘵𝘦𝘥 𝘉𝘺 trito シ︎', imageMessage: imageMsg,
              contentText:`Jangan Lupa Makan Bang ☹︎`,buttons,headerType:4}
...
sendButMessage(from, tiyo, `𝘊𝘳𝘦𝘢𝘵𝘦𝘥 𝘉𝘺 trito シ︎`, [
            {              
...
buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              ...
              buttonsMessage = {footerText:'𝘊𝘳𝘦𝘢𝘵𝘦𝘥 𝘉𝘺 trito シ︎', imageMessage: imageMsg,
              contentText:`𝗝𝗔𝗡𝗚𝗔𝗡 𝗟𝗨𝗣𝗔 𝗗𝗢𝗡𝗔𝗦𝗜☹︎`,buttons,headerType:4}
              prep = await itsmevall.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              itsmevall.relayWAMessage(prep)
break
case 'anal':
...
case 'nsfw':
		...
		case 'maker2d4': 
			...
			case 'battlefield': 
					...
					ll2 = makell.split("|")[1];
					reply(mess.wait)
					anu = await fetchJson(`https://api-xchillds.herokuapp.com/api
